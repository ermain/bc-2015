...
public class HQ extends OurRobot {
	...
	public void doTurn() throws GameActionException {
		...
		mRc.broadcast(Const.numMinerFactoryPos, numFactories);
		mRc.broadcast(Const.numMinerPos, numMiners);
		mRc.broadcast(Const.numBarracksPos, numBarracks);
		mRc.broadcast(Const.numTankFactoryPos, numTankFactory);
		

		

		determineTarget(enemyHQLoc, myHQLoc, enemyTowers, myTowers, numDrones, numDronesAtTarget, numTanksAtTarget, attacking, retreatTimer);
	...
	void handleSupply(RobotController rc)  throws GameActionException {
		...
		for (RobotInfo r : myRobots) {
			...
			} else if (r.type == RobotType.TANK && supply < Const.tankSupplyThreshold && mySupply >= Const.tankSupplyThreshold - supply) {
				rc.transferSupplies((int)(Const.tankSupplyThreshold - supply), r.location);
	...
	static boolean determineTarget(MapLocation enemyLoc, MapLocation myLoc, MapLocation [] enemyTowers, MapLocation [] myTowers, 
			int numDrones, int numDronesAtTarget, int numTanksAtTarget, int attacking, int retreatTimer) throws GameActionException, NullPointerException {
		int minDist = 999999;
		...
		boolean callRetreat = (attacking == 1) && (numDronesAtTarget < Const.massDroneMinThreshold && numTanksAtTarget < Const.massTankHQThreshold) && (retreatTimer <= 0);
		
		if (callRetreat) { // initiate retreat
			loc = enemyLoc;
			towers = myTowers;
			attacking = 0;
		} else if (enemyTowers.length <= Const.towersToLeave) {
			if (numDronesAtTarget > Const.massDroneHQThreshold || numTanksAtTarget > Const.massTankHQThreshold) { // attack their HQ
		...
		} else if (attacking == 1 || numDronesAtTarget > Const.massDroneMaxThreshold || numTanksAtTarget > Const.massTankMaxThreshold) { // initiate attack
			loc = myLoc;
			towers = enemyTowers;
			attacking = 1;
		} else { // status quo
